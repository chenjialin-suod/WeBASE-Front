...
//        }
     ...
     */
    public Block getBlockByNumber(String groupId, BigInteger blockNumber, boolean fullTrans) {
        if (blockNumberCheck(groupId, blockNumber)) {
            throw new FrontException(ConstantCode.BLOCK_NUMBER_ERROR);
        }
        Block block = getWeb3j(groupId).getBlockByNumber(blockNumber, false, fullTrans)
                .getBlock();
        return block;
    }
     ...
     */
    public Block getBlockByHash(String groupId, String blockHash, boolean fullTrans) {
        try {
            Block block = getWeb3j(groupId).getBlockByHash(blockHash,
                false, fullTrans)
                .getBlock();
            return block;
    ...
    public TransactionCountInfo getTransCnt(String groupId) {
        TransactionCountInfo transactionCount = getWeb3j(groupId)
